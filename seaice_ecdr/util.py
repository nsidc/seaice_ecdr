...
''' This is a WIP to refactor the assignment of TB arrays to a routine
def create_tb_dataarray(
    tb_data,
    dims,
    extra_attrs: dict = {},
    extra_encoding: dict = {},
    ...
    """Create DataArray with standard attributes and encoding for TBs"""
    # TODO: handle either np or xr data fields as input
    # TODO: handle either 2d or 3d data as input
    tb_valid_min = 10.0
    tb_valid_max = 350.0
    if len(tb_data.shape) == 2:
        tb_data_arr = np.expand_dims(tb_data, axis=0),
    else :
        tb_data_arr = tb_data
    tb_data_dtype= tb_data.dtype
    ...
    if packing is None:
        # Data are not to be packed
        valid_range_val = np.array((tb_valid_min, tb_valid_max), dtype=tb_data_dtype)
        da_attrs['valid_range'] = valid_range_val
    elif packing_type := type(packing) == bool:
        if packing_type:
            # Data are to be packed using default settings
            add_offset = np.float32(0.0).astype(tb_data_dtype)
            scale_factor = np.float32(0.1).astype(tb_data_dtype)
            valid_min = (tb_valid_min - add_offset) / scale_factor
            valid_max = (tb_valid_max - add_offset) / scale_factor
            pack_dtype = np.uint16
            valid_range_val = np.array((valid_min, valid_max), dtype=packing_dtype)

            da_attrs['valid_range'] = valid_range_val

            da_encoding['add_offset'] = add_offset
            da_encoding['scale_factor'] = scale_factor
            da_encoding['dtype'] = packing_dtype
        else:
            # Data are not to be packed
            valid_range_val = np.array((tb_valid_min, tb_valid_max), dtype=tb_data_dtype)
            da_attrs['valid_range'] = valid_range_val
    else:
        # Data are to be packed with non-standard settings
        # Expect dict keys:
        #   pack_dtype
        #   scale_factor
        #   add_offset (optional, defaults to zero)
        if packing is not None:
            if 'add_offset' in packing.keys():
                add_offset = packing['add_offset']
            else:
                add_offset = 0
            if 'scale_factor' in packing.keys():
                scale_factor = packing['scale_factor']
            else:
                scale_factor = 1

            if 'packing_dtype' in packing.keys():
                packing_dtype = packing['packing_dtype']
            else:
                packing_dtype = np.uint16
        else:
            add_offset = 0
            scale_factor = 1
            packing_dtype = np.uint16
        ...
        da_attrs['add_offset'] = add_offset
        da_attrs['scale_factor'] = scale_factor
        da_attrs['valid_range'] = valid_range_val


    # Encode the brightness temperatures
    tb_dataarray = xr.DataArray(
        {"time", "y", "x"},
        tb_data_arr,
        da_attrs,
        da_encoding,
